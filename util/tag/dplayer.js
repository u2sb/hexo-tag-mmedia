...
class DplayerTag extends BaseTag {
    ...
    parse(options) {
        let settings = Object.assign({}, DPLAYER_TAG_OPTION, this.dplayerConfig.default);
    ...
    generate() {
        let { url, pic, thumbnails, type,
            id, api, addition, token, maximum, user, bottom, unlimited,
            suburl, subtype, subbottom, subcolor, subfontSize,
            autoplay, loop, screenshot, theme, lang, hotkey, preload, logo, volume, mutex, contextmenu, highlight,
            code } = this.settings;
        ...
        subtitle = (suburl == null || suburl == '')
            ? ''
            : `subtitle: { url: '${suburl}', type: '${subtype}', fontSize: '${subfontSize}', bottom: '${subbottom}', color: '${subcolor}'},`;
        dankamu = (id == null || id == '')
        ?'{}'
        :`danmaku: {id: '${id}', api: '${api}', token: '${token}', maximum: '${maximum}', addition: '${addition}', user: '${user}', bottom: '${bottom}', unlimited: '${unlimited}'},`;
        tag += `
        <script>
        const dp = new DPlayer({
            ...
            theme: '${theme}',
            loop: ${loop},
            lang: ${(lang == null || lang == '') ? 'navigator.language': `'${lang}'`},
            ...
            ${subtitle}
            ${dankamu}
            contextmenu: ${contextmenu.length > 0 ? JSON.stringify(contextmenu) : "[{text: 'hexo-tag-mmedia', link: 'https://github.com/MonoLogueChi/hexo-tag-mmedia'}]"},
            highlight: ${highlight.length > 0 ? JSON.stringify(highlight) : "[]"}
        });
        ${code}
        </script>
        `;
